...
def RunModel(trained_model, imgPath, imgSizeSmall, imgSizeBig, img_offset, img_scale):
    ...
    # Convert to grayscale and scale the values to match input
    img_output_gray = cv2.cvtColor(img_output, cv2.COLOR_RGB2GRAY)
    min_input, max_input = np.min(img_input_show), np.max(img_input_show)
    min_output, max_output = np.min(img_output_gray), np.max(img_output_gray)
    img_output_gray = ((img_output_gray - min_output) / (max_output - min_output) * (max_input - min_input) + min_input).astype(np.uint8)

    # Replicate grayscale channel to make it 3-channel
    img_output_gray = np.stack([img_output_gray]*3, axis=-1)

    # Concatenate all of them
    img_tot = np.concatenate([img_groundtruth, img_input_show, img_output_gray], axis=1)
